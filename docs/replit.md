...
- **Prerender Optimization**: Default deployment uses priority mode (~50 critical SEO pages) for fast, reliable builds. For full 310+ page prerendering (slower, use locally): `PRERENDER_MODE=full npm run build`. Prerender script optimized with 10x concurrency, retry logic (2 retries per page), and 15s timeouts. Verification scripts are mode-aware to prevent false failures.
