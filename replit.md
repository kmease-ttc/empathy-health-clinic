...
### UI/UX Decisions
The frontend is a responsive React SPA built with TypeScript, Tailwind CSS, and Shadcn UI. It features a professional healthcare design, full dark mode support, and the Inter font. Key UI/UX decisions include aggressive cross-linking, prominent trust factors, and high-conversion lead capture forms. **Trust badge components** include a compact "Verified On" badge (`VerifiedOnBadge.tsx`) displaying platform verification logos (Healthgrades, Zocdoc, Google) in a horizontal pill format, reusable across landing pages for above-the-fold credibility.
...
- **Location Pages:** 11 city-specific landing pages optimized for local SEO, featuring unique content and LocalBusiness schema. The **Winter Park location page** (`/locations/winter-park`) features a comprehensive high-conversion design with all homepage trust elements including: Google rating badge, insurance logos, verification platforms badge (above-the-fold), trust factors grid, comparison section, approach timeline, curated team section (4 featured providers), curated testimonials (3 reviews), and interactive Google Maps embed positioned above the footer for optimal UX flow. Request Appointment buttons styled in green for high visibility.
